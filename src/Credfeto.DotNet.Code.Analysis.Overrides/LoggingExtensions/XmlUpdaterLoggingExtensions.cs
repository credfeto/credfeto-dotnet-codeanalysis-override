...
internal static partial class XmlUpdaterLoggingExtensions
{
    ...
    [LoggerMessage(EventId = 2, Level = LogLevel.Information, Message = "Rule {ruleSet}/{rule} ({name}) already set to {setting}")]
    ...
    [LoggerMessage(EventId = 3, Level = LogLevel.Information, Message = "Rule {ruleSet}/{rule} ({name}) changing from {existingSetting} to {newSetting}")]
